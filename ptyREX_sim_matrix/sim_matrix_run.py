...
def main(xyz, pixel_size, sim_npy, root_path, script_path, dose):
    """
    ...
    xyz: str
       full path of the xyz coordination file
    pixel_size: float
        pixel size of the sim
    sim_npy: str
       full path of the npy file of array with the shape (3, n) with n the total number conditions under consideration
    ...
    script_path: str
       full path of the cluster script submit directory
    dose: int
       target sum intensity of full 4DSTEM dataset

    Returns
    -------
    """
    sim_conditions = np.load(sim_npy)
    if not os.path.exists(root_path):
        os.makedirs(root_path)

    for i in np.arange(sim_conditions.shape[1]):
        sim_file = run_sim(script_path, root_path, xyz, pixel_size, sim_conditions[0,i], sim_conditions[1,i], sim_conditions[2,i])
...
if __name__ =='__main__':
    ...
    parser.add_argument('pixel_size', help='pixel size of sim')
    parser.add_argument('sim_npy', help='path for npy file with the sim conditions')
    ...
    parser.add_argument('dose', help='int, target sum full dataset')
    ...
    main(args.xyz, args.pixel_size, args.sim_npy, args.root_path, args.script_path, args.dose)
